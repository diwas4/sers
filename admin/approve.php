...
<?php
  ...
  {
    mysqli_query($db,"UPDATE  `issue_eqp` SET  `approve` =  '$_POST[approve]', `issue` =  '$_POST[issue]', 
    `return` =  '$_POST[return]' WHERE username='$_SESSION[name]' AND equipment_id='$_SESSION[equipment_id]';");

    mysqli_query($db,"UPDATE equipment SET quantity_available = quantity_available-1 where 
    equipment_id='$_SESSION[equipment_id]' ;");

    $res=mysqli_query($db,"SELECT quantity_available from equipment where 
    equipment_id='$_SESSION[equipment_id]';");
    ...
    while($row=mysqli_fetch_assoc($res))
    {
      ...
      {
        mysqli_query($db,"UPDATE equipment SET status='not-available' where 
        equipment_id='$_SESSION[equipment_id]';");
      }
    }
    ?>
