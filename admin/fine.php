...
<div class="container">
	...
	<?php
        ...
        {
            mysqli_query($db,"UPDATE `fine` SET status='paid' WHERE username='$_POST[username]' AND equipment_id='$_POST[equipment_id]' AND returned='$_POST[returned]'");
        }
			...
			while($row=mysqli_fetch_assoc($q))
			{
				...
				if($row['status'] == 'not paid') {
				    echo "<form method='post' action=''>
				          ...
				          <input type='hidden' name='returned' value='".$row['returned']."'>
				          <button type='submit' name='pay_fine' class='btn btn-success' style='background-color:green; color:white;'>Mark Paid</button>
				          </form>";
				}
				echo "</td>";
